...
; ================         move files from sub-directories to main     ================
	...
	EndIf
	If StringRegExp($g_Flags[14], 'BWS|BG1EE|BG2EE|PSTEE|IWD1EE') And FileExists($g_GameDir&'\SwordCoastStratagems-33.4') Then
		FileWrite($g_LogFile, '>SwordCoastStratagems-33.4\* .' & @CRLF)
		_Extract_MoveMod('SwordCoastStratagems-33.4')
	EndIf
	If StringRegExp($g_Flags[14], 'BWS|BG1EE|BG2EE|PSTEE|IWD1EE') And FileExists($g_GameDir&'\Tamoko-0.5') Then
		FileWrite($g_LogFile, '>Tamoko-0.5\* .' & @CRLF)
		_Extract_MoveMod('Tamoko-0.5')
	EndIf
