...
; ================         move files from sub-directories to main     ================
	...
	EndIf
	If StringRegExp($g_Flags[14], 'BWS|BG1EE|BG2EE|PSTEE|IWD1EE') And FileExists($g_GameDir&'\EEUITweaks-3.8') Then
		FileWrite($g_LogFile, '>EEUITweaks-3.8\* .' & @CRLF)
		_Extract_MoveMod('EEUITweaks-3.8')
	EndIf
	...
	EndIf
	If StringRegExp($g_Flags[14], 'BWS|BG1EE|BG2EE|PSTEE|IWD1EE') And FileExists($g_GameDir&'\Tweaks-Anthology-10') Then
		FileWrite($g_LogFile, '>Tweaks-Anthology-10\* .' & @CRLF)
		_Extract_MoveMod('Tweaks-Anthology-10')
	EndIf
